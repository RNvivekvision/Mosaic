...
const Blazer = ({ blazer, onConfirm }) => {
  const [State, setState] = useState({
    size: blazer?.size ?? null,
    tendToRun: blazer?.tendToRun ?? null,
  });
  ...
  return (
    <View style={RNStyles.container}>
      <Reanimated.View entering={FadeInDown.delay(200)}>
        <RNImage source={Images.section_6} style={selectionStyles.img} />
      </Reanimated.View>
      ...
      <Reanimated.View
        ...
        <Sizes
          title={'For Blazer'}
          data={DummyData.selection.blazer}
          selected={State.size}
