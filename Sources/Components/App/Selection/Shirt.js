...
const Shirt = ({ shirt, onConfirm }) => {
  const [State, setState] = useState({
    size: shirt?.size ?? null,
    tendToRun: shirt?.tendToRun ?? null,
  });
  ...
  return (
    <View style={RNStyles.container}>
      <Reanimated.View entering={FadeInDown.delay(200)}>
        <RNImage source={Images.section_4} style={selectionStyles.img} />
      </Reanimated.View>
      ...
      <Reanimated.View
        ...
        <Sizes
          title={'For Shirt'}
          data={DummyData.selection.shirt}
          selected={State.size}
